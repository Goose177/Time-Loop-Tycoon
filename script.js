...
// New Upgrades
const upgrades = [
  { name: "Time Extender", cost: 100, effect: () => { timeLeft += 360; } }, // +6 hours
  { name: "Efficiency Boost", cost: 200, effect: () => { workEarnings *= 2; } }, // Double earnings
  { name: "Investor Funding", cost: 500, effect: () => { money += 1000; } }, // +$1000
];

// Add Upgrade Buttons Dynamically
const upgradesContainer = document.createElement('div');
upgradesContainer.className = 'upgrades';
document.querySelector('.actions').appendChild(upgradesContainer);
...
upgrades.forEach(upgrade => {
  ...
  button.addEventListener('click', () => {
    if (money >= upgrade.cost) {
      money -= upgrade.cost;
      upgrade.effect();
      messageDisplay.textContent = `Upgrade purchased: ${upgrade.name}!`;
      updateDisplay();
    } else {
